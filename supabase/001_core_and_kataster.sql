...
begin
  ...
  company:=nullif(trim(coalesce(new.raw_user_meta_data->>'company_name','')),'');
  if company is null then company:='Meine Organisation'; end if;
  insert into public.organizations(name,created_by) values(company,new.id) returning id into org_id;
  insert into public.organization_members(organization_id,user_id,role,status) values(org_id,new.id,'owner','active');
  return new;
end; $$;
...
create policy profiles_self_or_same_org on public.profiles for select to authenticated using(
 id=auth.uid() or exists(select 1 from public.organization_members a join public.organization_members b on b.organization_id=a.organization_id where a.user_id=auth.uid() and a.status='active' and b.user_id=profiles.id and b.status='active')
);
...
create policy organization_members_member_select on public.organization_members for select to authenticated using(public.is_org_member(organization_id));
drop policy if exists organization_members_admin_manage on public.organization_members;
create policy organization_members_admin_manage on public.organization_members for all to authenticated using(public.has_org_role(organization_id,array['owner','admin'])) with check(public.has_org_role(organization_id,array['owner','admin']));
...
create trigger hazardous_substances_updated_at before update on public.hazardous_substances for each row execute function public.set_updated_at();
...
create policy substances_select_member on public.hazardous_substances for select to authenticated using(public.is_org_member(organization_id));
drop policy if exists substances_insert_editor on public.hazardous_substances;
create policy substances_insert_editor on public.hazardous_substances for insert to authenticated with check(public.has_org_role(organization_id,array['owner','admin','ersteller']));
drop policy if exists substances_update_editor on public.hazardous_substances;
create policy substances_update_editor on public.hazardous_substances for update to authenticated using(public.has_org_role(organization_id,array['owner','admin','ersteller','pruefer'])) with check(public.has_org_role(organization_id,array['owner','admin','ersteller','pruefer']));
drop policy if exists substances_delete_admin on public.hazardous_substances;
create policy substances_delete_admin on public.hazardous_substances for delete to authenticated using(public.has_org_role(organization_id,array['owner','admin']));
